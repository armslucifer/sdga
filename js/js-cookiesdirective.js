...
;(function($) {
	...
	var disclosure = function(options) {
		...
		if (!settings.customDialogSelector) {
			...
			var html = '';
			html += '<div id="epd">';
			html += '<div id="cookiesdirective" style="opacity: 1!important; position:'+ settings.css +';'+ settings.position + ':-300px;left:0px;width:100%;';
			html += 'height:auto;background:' + settings.backgroundColor + ';opacity:.' + settings.backgroundOpacity + ';';
			html += '-ms-filter: “alpha(opacity=' + settings.backgroundOpacity + ')”; filter: alpha(opacity=' + settings.backgroundOpacity + ');';
			html += '-khtml-opacity: .' + settings.backgroundOpacity + '; -moz-opacity: .' + settings.backgroundOpacity + ';';
			html += 'color:' + settings.fontColor + ';font-family:' + settings.fontFamily + ';font-size:' + settings.fontSize + ';';
			html += 'z-index:1000;">';
			html += '<div class="cookies--col">';
			
			if(!settings.message) {
				if(settings.explicitConsent) {
					// Explicit consent message
					settings.message = 'Le site SDGA utilise des cookies. Certains de ces cookies ';
					...
					settings.message = 'Le site SDGA utilise des cookies. Certains de ces cookies que nous utilisons sont essentiels au bon fonctionnement de certaines parties du site et ont déjà été configurés.<br/>';
				}
			}
			...
			if(settings.explicitConsent) {
				...
				html += scriptsDisclosure + ' Plus d\'informations sur la page <a class="hover-link" style="color:'+ settings.linkColor + ';';
				html += 'font-family:' + settings.fontFamily + ';font-size:' + settings.fontSize + ';" href="'+ settings.privacyPolicyUri + '">RGPD</a>.';
			...
			// the message again.
				...
				if ($cd.find('input#impliedsubmit').length == 0) {
					$cd.append('<input type="submit" class="small" id="impliedsubmit" value="Do not show this message again" />');
